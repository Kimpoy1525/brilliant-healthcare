...
document.getElementById("loginForm").addEventListener("submit",async event=>{event.preventDefault();const error=document.getElementById("loginError");error.textContent="";try{await fetch("/api/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(Object.fromEntries(new FormData(event.target)))}).then(async r=>{const d=await r.json();if(!r.ok)throw new Error(d.error);return d});event.target.reset();await boot()}catch(e){error.textContent=e.message}});
logout.addEventListener("click",async()=>{try{await api("/api/logout",{method:"POST"})}catch{}showLogin()});document.getElementById("refresh").addEventListener("click",boot);
let adminDoctors=[];
...
async function loadDoctors(){adminDoctors=await api("/api/admin/doctors");const list=document.getElementById("doctorList");list.replaceChildren();if(!document.getElementById("adminWeekSchedule").children.length)buildWeek([],"adminWeekSchedule");adminDoctors.forEach(d=>{const row=document.createElement("div");row.className="doctor-row";const meta=document.createElement("div");meta.className="doctor-meta";const name=document.createElement("strong");name.textContent=`${d.name} — ${d.specialty}`;const schedule=document.createElement("small");schedule.textContent=`${d.active?"Active":"Inactive"} · ${scheduleSummary(d)}`;meta.append(name,schedule);const buttons=document.createElement("div");buttons.className="doctor-buttons";const edit=document.createElement("button");edit.type="button";edit.className="button-link admin-link";edit.textContent="Edit schedule";edit.onclick=()=>editDoctor(d);const toggle=document.createElement("button");toggle.type="button";toggle.className="button-link admin-link";toggle.textContent=d.active?"Deactivate":"Activate";toggle.onclick=async()=>{await api(`/api/admin/doctors/${d.id}`,{method:"PATCH",body:JSON.stringify({active:!d.active})});notify("Doctor updated.");loadDoctors()};buttons.append(edit,toggle);row.append(meta,buttons);list.append(row)})}
function editDoctor(doctor){const form=document.getElementById("doctorForm");form.elements.doctorId.value=doctor.id;form.elements.name.value=doctor.name;form.elements.specialty.value=doctor.specialty;form.elements.unavailableDates.value=(doctor.unavailableDates||[]).join(", ");buildWeek(doctor.availability,"adminWeekSchedule");document.getElementById("doctorFormTitle").textContent="Edit doctor and schedule";document.getElementById("doctorSubmitText").textContent="Save doctor and schedule";document.getElementById("cancelDoctorEdit").hidden=false;document.getElementById("adminTools").scrollIntoView({behavior:"smooth"})}
function resetDoctorForm(){const form=document.getElementById("doctorForm");form.reset();form.elements.doctorId.value="";buildWeek([],"adminWeekSchedule");document.getElementById("doctorFormTitle").textContent="Add a doctor and schedule";document.getElementById("doctorSubmitText").textContent="Add doctor and publish schedule";document.getElementById("cancelDoctorEdit").hidden=true}
document.getElementById("cancelDoctorEdit").addEventListener("click",resetDoctorForm);
document.getElementById("doctorForm").addEventListener("submit",async event=>{event.preventDefault();const form=event.target,doctorId=form.elements.doctorId.value;const availability=readWeek("adminWeekSchedule"),unavailableDates=form.elements.unavailableDates.value.split(",").map(x=>x.trim()).filter(Boolean);if(!availability.length){notify("Select at least one working day.");return}const body={name:form.elements.name.value,specialty:form.elements.specialty.value,availability,unavailableDates};try{if(doctorId)await api(`/api/admin/doctors/${doctorId}`,{method:"PATCH",body:JSON.stringify(body)});else{const doctor=await api("/api/admin/doctors",{method:"POST",body:JSON.stringify(body)});await api(`/api/admin/doctors/${doctor.id}`,{method:"PATCH",body:JSON.stringify(body)})}resetDoctorForm();notify(doctorId?"Doctor and schedule updated.":"Doctor and schedule published.");loadDoctors()}catch(e){notify(e.message)}});
