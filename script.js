...
const serviceDetails = {
    ...
    "lab-diagnostics": { kicker: "Diagnostic services", title: "Lab Diagnostics", summary: "Clinical laboratory testing that supports screening, diagnosis, treatment monitoring, and ongoing health management.", about: "Available testing may include blood studies, urinalysis, chemistry panels, and other clinician-requested investigations. Test selection depends on your consultation or request form.", expect: ["Identity and test-order verification", "Safe specimen collection and labeling", "Results released through the clinic's approved process"], prepare: "Some tests require fasting or timed collection. Confirm instructions with the clinic before your visit and bring the physician request, if applicable." },
    "cardiac": { kicker: "Heart health", title: "Cardiac Diagnostics", summary: "Non-invasive assessments that help clinicians evaluate heart rhythm, structure, and cardiovascular response.", about: "Services may include ECG and other physician-requested cardiac studies. The exact examination and interpretation process depends on your symptoms and referral.", expect: ["Brief medical-history and symptom review", "Preparation and placement of monitoring equipment", "Clinical review or referral for interpretation"], prepare: "Wear comfortable clothing, bring your medication list, and follow any instructions about caffeine, meals, or activity provided for your specific test." },
    "radiology": { kicker: "Medical imaging", title: "Radiology & Imaging", summary: "Imaging services that help clinicians examine internal structures and support accurate clinical decisions.", about: "Available examinations may include digital X-ray and ultrasound. A referral and clinical indication may be required depending on the requested study.", expect: ["Referral and safety screening", "Positioning by trained imaging personnel", "Images prepared for qualified clinical interpretation"], prepare: "Bring your request form and previous related images or reports. Tell staff about pregnancy, implants, allergies, or other relevant safety concerns." },
    "checkup": { kicker: "Preventive care", title: "Health Checkups", summary: "A structured health review designed to identify risk factors early and guide appropriate preventive care.", about: "A checkup may combine history, vital signs, physical assessment, and selected laboratory or diagnostic tests based on age, risks, and clinical needs.", expect: ["Health history and risk-factor review", "Basic physical and vital-sign assessment", "Personalized next-step recommendations"], prepare: "Bring your medication list, vaccination history, and previous results. Ask whether your selected screening package requires fasting." }
};
...
function openServiceModal(card) { const detail = serviceDetails[card.dataset.service]; if (!detail) return; modalTrigger = card; document.getElementById("serviceModalKicker").textContent = detail.kicker; document.getElementById("serviceModalTitle").textContent = detail.title; document.getElementById("serviceModalSummary").textContent = detail.summary; document.getElementById("serviceModalAbout").textContent = detail.about; document.getElementById("serviceModalPrepare").textContent = detail.prepare; const list = document.getElementById("serviceModalExpect"); list.replaceChildren(); detail.expect.forEach(item => { const li = document.createElement("li"); li.textContent = item; list.append(li); }); document.getElementById("serviceModalBook").dataset.service = card.dataset.service; serviceModal.hidden = false; document.body.classList.add("modal-open"); modalPanel.focus(); }
function closeServiceModal() { serviceModal.hidden = true; document.body.classList.remove("modal-open"); modalTrigger?.focus(); }
...
document.getElementById("serviceModalBook").addEventListener("click", function () { form.elements.service.value = this.dataset.service; const james = doctors.find(d => d.name.includes("James Raphael")); if (james) { doctorSelect.value = james.id; doctorSelect.dispatchEvent(new Event("change")); } closeServiceModal(); });
